...
class Stack{
	int* dataArray;
	int top;			// Queue에서 enQueue 로 데이터를 저장할 index
...
public:
	...
	void push(int data);  // Queue에 데이터를 저장
	int pop();    // Queue에 저장된 데이터 중 가장 오래된 데이터의 값을 반환하고 저장 장소의 데이터를 삭제함
	int Front();      // Queue 제일 앞에 있는 데이터 반환 (데이터 삭제하지 않음)
	int Top();       // Queue 제일 뒤에 있는 데이터 반환 (데이터 삭제하지 않음)	int size() {return size;}
